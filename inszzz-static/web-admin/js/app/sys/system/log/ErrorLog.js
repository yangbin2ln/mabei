 ...
 * 系统错误日志
 */
Ext.define('App.sys.system.log.ErrorLog', {
	...
	init : function () {
		...
		var view = Ext.create('Ins.grid.Grid',{
			hideTitle: true,
			url: CTX_PATH + '/system/log/loadAll?type=2',
			fields: ['id','createUserName','createTime','account','ip','menuName','operateName','execTimemillis','params','errorDetails','departmentName','companyName'],
			condition : {
				fields : [{
					text: '模块',
					name: 'menuName'
			...
			valueMapper: {				
				params: function(value, v, record, index){
					return "<a href='javascript:App.getClass(\"App.sys.system.log.ErrorLog\").viewParams(\""+record.data.id+"\")' style='color:blue'>[参数明细]</a>";	
				},
				errorDetails: function(value, v, record, index){
					return "<a href='javascript:App.getClass(\"App.sys.system.log.ErrorLog\").viewErrorLog(\""+record.data.id+"\")' style='color:blue'>[错误明细]</a>";	
				}
	...
	viewErrorLog	: function(id){
		...
		Ext.create('Ins.window.Window',{
			...
			modal: true,
			maximizable: true,
			views:[{scrollable: true,html: record.data.errorDetails,  bodyPadding: 10}]
		}).show();
